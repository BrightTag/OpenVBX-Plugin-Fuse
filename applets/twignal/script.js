jQuery(function($) {
    ...
    $('.signal-applet tr.hide input').attr('disabled', 'disabled');

    //This function will create a new row for the table based off 
    // the template in the footer
    $('.signal-applet .action.add').live('click', function(event) {
        event.preventDefault();
        var row = $(this).closest('tr'),
            wrapper = $(this).parents('.signal-applet'),
    ...
    $('.signal-applet .action.remove').live('click', function() {
        var row = $(this).closest('tr'),
    ...
    $('.signal-applet .options-table').live('change', function() {
        $('tbody tr', this).first()
            .find('.action.remove').hide();
    });

    $('.signal-applet .options-table').trigger('change');
});